...
class BackprojectDepthRA(nn.Module):
    """Layer to transform a depth image into a point cloud
    """
    def __init__(self, batch_size, height, width, device):
        super(BackprojectDepthRA, self).__init__()
    ...
    def forward(self, depth, inv_K, dxy):
        ...
        for i in range(self.batch_size):
            ...
            ones = nn.Parameter(torch.ones(1, 1, self.height * self.width),
                                     requires_grad=False).to(self.device)  #[1,1,122880]

            pix_coords = torch.unsqueeze(torch.stack(
                [id_coords[0].view(-1), id_coords[1].view(-1)], 0), 0).to(self.device)    #[1,2,122880]
            ...
            pix_coords = nn.Parameter(torch.cat([pix_coords, ones], 1),
                                           requires_grad=False).to(self.device)   #[1,3,122880]
...
class Project3DRA(nn.Module):
    """Layer which projects 3D points into a camera with intrinsics K and at position T
    """
    def __init__(self, batch_size, height, width, eps=1e-7):
        super(Project3DRA, self).__init__()
