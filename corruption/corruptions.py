...
def pixelate(x, severity=1):
    count = [0.6, 0.5, 0.4, 0.3, 0.25][severity - 1]

    x_shape = x.size

    # x = x.resize((int(x_shape[1] * count), int(x_shape[0] * count)), Image.BOX)
    x = x.resize((int(x_shape[0] * count), int(x_shape[1] * count)), resample=Image.Resampling.BILINEAR)

    # x = x.resize((x_shape[1], x_shape[0]), Image.NEAREST)
    x = x.resize((x_shape[0], x_shape[1]), Image.Resampling.NEAREST)
