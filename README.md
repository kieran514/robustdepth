...
#### NuScenes-Night
For this testing, we provide the test dataset itself with the ground truth for simpplciity. Additions have been made to the code to allow for these evalautions so make sure you are uop to date. First, download the [NuScenes_test.zip](https://drive.google.com/file/d/1zeZ2pMnsXLn79_qPcO7dgbEbhbZ2-J1z/view?usp=sharing) and extract this to the data path. This dataset is under the same licnce as defined in [NuScenes](https://www.nuscenes.org/nuscenes) Creative Commons Attribution-NonCommercial-ShareAlike 4.0 International Public License (“CC BY-NC-SA 4.0”). This is not meant for commercial use and all credit for the data goes to [NuScenes](https://www.nuscenes.org/nuscenes). An individaul Licence is provided with the zipped data.

Next download [gt_depths.zip](https://drive.google.com/file/d/1kXsZ7DbcEUO4LueHvv2AqvF8PkWbW6OX/view?usp=sharing) and extract these files into in splits/nuScenes_test_night/. Now we can simply run:
```
