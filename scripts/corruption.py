        ...
        for sync in os.listdir(sequences):
            if sync[-4:] == 'sync':
                ...
                for image in sorted(os.listdir(img_path)):
                    ...
                    output_gaussian_noise = os.path.join(sequences, sync, 'image_02/gaussian_noise/')
                    output_shot_noise = os.path.join(sequences, sync, 'image_02/shot_noise/')
                    output_impulse_noise = os.path.join(sequences, sync, 'image_02/impulse_noise/')
                    output_defocus_blur = os.path.join(sequences, sync, 'image_02/defocus_blur/')
                    output_glass_blur = os.path.join(sequences, sync, 'image_02/glass_blur/')
                    output_zoom_blur = os.path.join(sequences, sync, 'image_02/zoom_blur/')
                    output_snow = os.path.join(sequences, sync, 'image_02/snow/')
                    output_frost = os.path.join(sequences, sync, 'image_02/frost/')
                    output_elastic_transform = os.path.join(sequences, sync, 'image_02/elastic_transform/')
                    output_pixelate = os.path.join(sequences, sync, 'image_02/pixelate/')
                    output_jpeg_compression = os.path.join(sequences, sync, 'image_02/jpeg_compression/')
                    output_grey = os.path.join(sequences, sync, 'image_02/greyscale/')
                    output_R = os.path.join(sequences, sync, 'image_02/R/')
                    output_G = os.path.join(sequences, sync, 'image_02/G/')
                    output_B = os.path.join(sequences, sync, 'image_02/B/')
